...
A: **忏悔+官中补丁：实测可共用，且无安装顺序要求**（先装哪个都行）。REPENTOGON：理论上兼容，但未联合实测，请自行验证。

- **忏悔+官中补丁**：实测可以与本 Mod 同时使用，**安装顺序无要求**——两者都在 `isaac-ng.exe` 导入表上**追加各自独立的条目**，互不影响、互不覆盖，谁先打补丁都可以。本 Mod 还自动兼容官中补丁的启动校验（将其 config.ini 的 check 改为 -1，消除弹窗）。
