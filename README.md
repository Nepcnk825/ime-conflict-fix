...
旧版本用户：请先删除旧版本解压出来的 `ime_patcher.exe` / `ime_loader.dll` 等文件，再解压本版本 `ime_loader.zip`，使用新的 `ime_patcher.exe` 先卸载/还原，然后重新打补丁安装。
...
A: 游戏会先重载 Mod；一旦游戏日志出现本 Mod 的执行记录，DLL 会立即请求英文布局。若游戏因此自动重启，下次启动会在主菜单直接生效。
...
### Q: 搜狗 / 小狼毫等输入法能用吗？
A: 目前实现方式是直接切换输入法，而不是禁用微软拼音的中文模式，所以理论上全部兼容。
