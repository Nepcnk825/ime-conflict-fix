...
有两种方式，**任选其一**即可（都需要打补丁步骤，两种方式的差异只在于如何获取 mod 本体）：

### 方式一：Steam 创意工坊订阅（推荐）

1. 在 Steam 创意工坊搜索 "IME Conflict Fix" 并订阅
2. 启动游戏，在主菜单 Mods 菜单确认 "IME Conflict Fix" 已启用
3. 从下方"下载工具"获取 `ime_patcher.exe` 和 `ime_fix.dll`，运行打补丁

### 方式二：直接复制 mod 文件夹

把本项目的 `ime-conflict-fix/` 文件夹（含 `main.lua` + `metadata.xml`）直接复制到游戏的 `mods/` 目录，然后启动游戏在 Mods 菜单启用。

### 下载工具（两种方式都需要）

由于本仓库只发布源代码（不提供预构建二进制，有意审查者可从源码自行构建），你可以：

- **自己构建**（需要 Visual Studio 2022）：
  ```bat
...
需要澄清的边界场景：如果你给 `isaac-ng.exe` 打了补丁（DLL 已注入），**然后**在游戏内手动开启了 Mod 又退出到主菜单再进联机——此时 DLL 可能已因检测到 mod 启用而调用过 `ImmDisableIME(-1)`（不可逆），联机内也无法恢复中文输入法。这是"启动即禁用"方案与联机机制的固有权衡：**联机游玩请保持 Mod 禁用状态启动**。
