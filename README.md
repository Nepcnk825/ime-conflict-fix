...
2. 启动游戏，在主菜单 **Mods** 菜单中找到 "IME Conflict Fix"（确认创意工坊已下载成功）
3. 退出游戏，运行 `ime_patcher.exe` 打补丁（见下方"打补丁"）
4. 重新启动游戏，在 Mods 菜单确认 "IME Conflict Fix" 已启用
...
4. 启动游戏，在主菜单 Mods 菜单确认 "IME Conflict Fix" 出现并启用
5. 退出游戏，运行 `ime_patcher.exe` 打补丁（见下方"打补丁"）
6. 重新启动游戏，确认补丁生效
...
A: **忏悔+官中补丁：实测可共用**。REPENTOGON：理论上兼容，但未联合实测，请自行验证。

- **忏悔+官中补丁**：实测可以与本 Mod 同时使用。两者都通过修改 `isaac-ng.exe` 导入表加载各自的 DLL，多个补丁工具在**同一个 exe 文件上逐个追加条目**，同时使用时**按安装顺序逐个运行各补丁工具**即可。本 Mod 还自动兼容官中补丁的启动校验（将其 config.ini 的 check 改为 -1，消除弹窗）。
- **REPENTOGON**：通过把 `dsound.dll` 等文件放入游戏目录来加载（DLL 代理方式），**不修改 `isaac-ng.exe`**。本 Mod 通过**修改 exe 导入表**加载 `ime_fix.dll`，两者机制独立，理论上互不冲突。但由于从未联合测试过，如果遇到问题，请尝试先单独运行各补丁工具，确认各自单独可用后再组合。
